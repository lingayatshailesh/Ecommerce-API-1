...
exports.userSignup = async function(req, res, next) {
  ...
  exports.userLogin = async function(req, res, next) {
    try {
      ...
      if(!userLogin) throw new Error("User not Found")
        let passwordComapre = await bcrypt.compare(req.body.password,userLogin.password)
      if(!passwordComapre) throw new Error("Invalid Password")
  ...
  exports.userFoundAllData = async function(req, res, next) {
    try {
      let userFoundAllData = await USER.find()

      res.status(200).json({
        status : "Success",
        message : "User All Data Found Successfully",
        data : userFoundAllData
      })
    } catch (error) {
        res.status(404).json({
            status : "Fail",
